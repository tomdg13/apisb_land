...
let ListingsService = class ListingsService {
    ...
    async getAll(query, userId = null) {
        try {
            const { page = 1, limit = 12, category_id, province_id, district_id, listing_type, status = 'active', min_price, max_price, is_featured, search, sort_by = 'created_date', sort_order = 'DESC', } = query;
            ...
            if (province_id) {
                conditions.push('l.province_id = ?');
                params.push(province_id);
            }
            if (district_id) {
                conditions.push('l.district_id = ?');
            ...
            const [countRow] = await this.dataSource.query(`SELECT COUNT(*) AS total FROM listings l WHERE ${where}`, params);
    ...
    async getOne(id) {
        try {
            await this.dataSource.query(`UPDATE listings SET views_count = views_count + 1 WHERE listing_id = ?`, [id]);
         ...
         WHERE l.listing_id = ?`, [id]);
            ...
            const images = await this.dataSource.query(`SELECT image_id, image_url, is_cover, sort_order
         FROM listing_images WHERE listing_id = ? ORDER BY sort_order`, [id]);
            const [landDetail] = await this.dataSource.query(`SELECT * FROM land_details WHERE listing_id = ?`, [id]);
            return {
                responseCode: '00',
    ...
    async create(dto, userId) {
        try {
            const [cat] = await this.dataSource.query(`SELECT category_id FROM categories WHERE category_id = ? AND is_active = 1`, [dto.category_id]);
            if (!cat)
                return { responseCode: '01', message: 'Category not found or inactive' };
            const result = await this.dataSource.query(`INSERT INTO listings
    ...
    async update(id, dto, userId, userRole) {
        try {
            const [listing] = await this.dataSource.query(`SELECT user_id, status FROM listings WHERE listing_id = ?`, [id]);
            if (!listing)
                return { responseCode: '01', message: 'Listing not found' };
            ...
            if (fields.length === 0)
                return { responseCode: '02', message: 'No fields to update' };
            params.push(id);
            await this.dataSource.query(`UPDATE listings SET ${fields.join(', ')} WHERE listing_id = ?`, params);
    ...
    async remove(id, userId, userRole) {
        try {
            const [listing] = await this.dataSource.query(`SELECT user_id FROM listings WHERE listing_id = ?`, [id]);
            if (!listing)
                return { responseCode: '01', message: 'Listing not found' };
            if (listing.user_id !== userId && !['Admin', 'Manager'].includes(userRole)) {
                return { responseCode: '03', message: 'Permission denied' };
    ...
    async approve(id) {
        try {
            await this.dataSource.query(`UPDATE listings SET status = 'active', reject_reason = NULL WHERE listing_id = ?`, [id]);
    ...
    async reject(id, reason) {
        try {
            await this.dataSource.query(`UPDATE listings SET status = 'rejected', reject_reason = ? WHERE listing_id = ?`, [reason, id]);
    ...
    async addImages(listingId, files, userId, userRole, appUrl) {
        try {
            const [listing] = await this.dataSource.query(`SELECT user_id FROM listings WHERE listing_id = ?`, [listingId]);
            if (!listing)
                return { responseCode: '01', message: 'Listing not found' };
            if (listing.user_id !== userId && !['Admin', 'Manager'].includes(userRole)) {
                return { responseCode: '03', message: 'Permission denied' };
            }
            const [countRow] = await this.dataSource.query(`SELECT COUNT(*) AS cnt FROM listing_images WHERE listing_id = ?`, [listingId]);
            ...
            for (let i = 0; i < files.length; i++) {
                ...
                const result = await this.dataSource.query(`INSERT INTO listing_images (listing_id, image_url, is_cover, sort_order)
           VALUES (?, ?, ?, ?)`, [listingId, imageUrl, isCover, existing + i]);
    ...
    async deleteImage(imageId, userId, userRole) {
        try {
            const [image] = await this.dataSource.query(`SELECT li.image_id, l.user_id
         FROM listing_images li
         JOIN listings l ON l.listing_id = li.listing_id
         WHERE li.image_id = ?`, [imageId]);
            if (!image)
                return { responseCode: '01', message: 'Image not found' };
    ...
    async setCover(imageId, userId, userRole) {
        try {
            const [image] = await this.dataSource.query(`SELECT li.image_id, li.listing_id, l.user_id
         FROM listing_images li
         JOIN listings l ON l.listing_id = li.listing_id
         WHERE li.image_id = ?`, [imageId]);
            if (!image)
            ...
            await this.dataSource.query(`UPDATE listing_images SET is_cover = 0 WHERE listing_id = ?`, [image.listing_id]);
            await this.dataSource.query(`UPDATE listing_images SET is_cover = 1 WHERE image_id = ?`, [imageId]);
    ...
    async saveLandDetail(listingId, dto, userId, userRole) {
        try {
            const [listing] = await this.dataSource.query(`SELECT user_id FROM listings WHERE listing_id = ?`, [listingId]);
            if (!listing)
                return { responseCode: '01', message: 'Listing not found' };
            ...
            await this.dataSource.query(`INSERT INTO land_details
    ...
    async toggleFavorite(listingId, userId) {
        try {
            const [existing] = await this.dataSource.query(`SELECT favorite_id FROM favorites WHERE user_id = ? AND listing_id = ?`, [userId, listingId]);
            if (existing) {
                await this.dataSource.query(`DELETE FROM favorites WHERE favorite_id = ?`, [existing.favorite_id]);
                return { responseCode: '00', message: 'Removed from favorites', is_favorite: false };
            }
            else {
                await this.dataSource.query(`INSERT INTO favorites (user_id, listing_id) VALUES (?, ?)`, [userId, listingId]);
    ...
    async getMyFavorites(userId) {
        try {
         ...
         LEFT JOIN provinces p ON p.province_id = l.province_id
         WHERE f.user_id = ?
         ORDER BY f.created_date DESC`, [userId]);
    ...
    async createInquiry(dto, userId) {
        try {
            const [listing] = await this.dataSource.query(`SELECT listing_id FROM listings WHERE listing_id = ? AND status = 'active'`, [dto.listing_id]);
            if (!listing)
                return { responseCode: '01', message: 'Listing not found or not active' };
            await this.dataSource.query(`INSERT INTO inquiries
    ...
    async getInquiries(listingId, userId, userRole) {
        try {
            const [listing] = await this.dataSource.query(`SELECT user_id FROM listings WHERE listing_id = ?`, [listingId]);
            if (!listing)
                return { responseCode: '01', message: 'Listing not found' };
            ...
            const data = await this.dataSource.query(`SELECT i.*, u.full_name AS user_name, u.avatar_url
         FROM inquiries i
         LEFT JOIN users u ON u.user_id = i.sender_id
         WHERE i.listing_id = ?
         ORDER BY i.created_date DESC`, [listingId]);
            await this.dataSource.query(`UPDATE inquiries SET is_read = 1 WHERE listing_id = ?`, [listingId]);
            return { responseCode: '00', data };
        }
        catch (error) {
            return { responseCode: '99', message: error.message };
    ...
    async getCategories() {
        try {
            const data = await this.dataSource.query(`SELECT category_id, name, name_en, icon
    ...
    async getProvinces() {
        try {
            const data = await this.dataSource.query(`SELECT province_id, province_name, province_code FROM provinces ORDER BY province_id`);
            return { responseCode: '00', data };
        }
        catch (error) {
            return { responseCode: '99', message: error.message };
    ...
    async getDistricts(provinceId) {
        try {
            const data = await this.dataSource.query(`SELECT district_id, district_name FROM districts
         WHERE province_id = ? ORDER BY district_id`, [provinceId]);
            return { responseCode: '00', data };
        }
        catch (error) {
            return { responseCode: '99', message: error.message };
    ...
    async getVillages(districtId) {
        try {
            const data = await this.dataSource.query(`SELECT village_id, village_name FROM villages
         WHERE district_id = ? ORDER BY village_id`, [districtId]);
            return { responseCode: '00', data };
        }
        catch (error) {
            return { responseCode: '99', message: error.message };
    ...
    async getPackages() {
        try {
            const data = await this.dataSource.query(`SELECT * FROM packages WHERE is_active = 1 ORDER BY price`);
            return { responseCode: '00', data };
        }
        catch (error) {
            return { responseCode: '99', message: error.message };
