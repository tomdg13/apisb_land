...
let ProfileService = class ProfileService {
    ...
    async getProfile(userId, viewerId = null) {
        try {
            const [user] = await this.dataSource.query(`SELECT
         ...
         WHERE u.user_id = ? AND u.is_active = 1`, [userId]);
            if (!user)
         ...
         FROM listings WHERE user_id = ? AND status != 'draft'`, [userId]);
            const listings = await this.dataSource.query(`SELECT
         ...
         ORDER BY l.created_date DESC LIMIT 6`, [userId]);
         ...
         WHERE r.seller_id = ?
         ORDER BY r.created_date DESC LIMIT 10`, [userId]);
            return {
                responseCode: '00',
                data: {
                    ...user,
    ...
    async updateProfile(userId, dto) {
        try {
            ...
            if (userFields.length > 0) {
                ...
                await this.dataSource.query(`UPDATE users SET ${userFields.join(', ')} WHERE user_id = ?`, userParams);
            }
            const profFields = [];
            const profParams = [];
            const setProfField = (col, val) => {
                if (val !== undefined) {
                    profFields.push(`${col} = ?`);
                    profParams.push(val);
                }
            ...
            if (profFields.length > 0) {
                ...
                await this.dataSource.query(`INSERT INTO user_profiles (user_id) VALUES (?)
           ON DUPLICATE KEY UPDATE ${profFields.join(', ')}`, [userId, ...profParams]);
            }
    ...
    async updateAvatar(userId, avatarUrl) {
        try {
            await this.dataSource.query(`UPDATE users SET avatar_url = ? WHERE user_id = ?`, [avatarUrl, userId]);
    ...
    async updateCoverUrl(userId, coverUrl) {
        try {
            await this.dataSource.query(`INSERT INTO user_profiles (user_id, cover_url) VALUES (?, ?)
         ON DUPLICATE KEY UPDATE cover_url = ?`, [userId, coverUrl, coverUrl]);
    ...
    async changePassword(userId, currentPassword, newPassword) {
        try {
            ...
            const [user] = await this.dataSource.query(`SELECT password_hash FROM users WHERE user_id = ?`, [userId]);
            if (!user)
                return { responseCode: '01', message: 'User not found' };
            const valid = await bcrypt.compare(currentPassword, user.password_hash);
            if (!valid)
                return { responseCode: '02', message: 'ລະຫັດຜ່ານປັດຈຸບັນບໍ່ຖືກຕ້ອງ' };
            const hash = await bcrypt.hash(newPassword, 10);
            await this.dataSource.query(`UPDATE users SET password_hash = ? WHERE user_id = ?`, [hash, userId]);
    ...
    async addReview(sellerId, reviewerId, rating, text, listingId) {
        try {
            ...
            await this.dataSource.query(`INSERT INTO user_reviews (seller_id, reviewer_id, listing_id, rating, review_text)
         VALUES (?, ?, ?, ?, ?)
         ON DUPLICATE KEY UPDATE rating = VALUES(rating), review_text = VALUES(review_text)`, [sellerId, reviewerId, listingId ?? null, rating, text ?? null]);
            await this.dataSource.query(`UPDATE user_profiles
         SET
         ...
         WHERE user_id = ?`, [sellerId, sellerId, sellerId]);
